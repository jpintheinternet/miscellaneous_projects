...
#   Running both 'players' with the algorithm will always end in a draw
...
def O_move(board):
    ...
    for row in range(len(board)):
        for col in range(len(board[row])):
            if board[row][col] == EMPTY:
                board[row][col] = O
                score = minimaxO(board, 0, False)  # check if input is depth or 0
                board[row][col] = EMPTY
                if (score > bestScore):
                    bestScore = score
                    bestMoveRow = row
...
def minimaxO(board, depth, isMaximizing):
    if evaluate_game(board) == GAME_O:
        return 10
    elif evaluate_game(board) == GAME_X:
        return -10
    ...
    if isMaximizing:
        ...
        for row in range(len(board)):
            for col in range(len(board[row])):
                if (board[row][col] == EMPTY):
                    board[row][col] = X
                    score = minimaxO(board, depth + 1, True)
                    board[row][col] = EMPTY
                    if (score < bestScore):
                        bestScore = score
        return bestScore
...
def X_move(board):
    ...
    # START FILLER CODE, just picks first valid move!   DELETE THIS ONE!
    # for row in range(len(board)):
    #     for col in range(len(board[row])):
    #         if board[row][col] == EMPTY:
    #             return (row, col)
    ...
    # START FILLER CODE, MINIMAX Algorithm!
    bestScore = -100
    bestMoveRow = 0
    bestMoveCol = 0
    depth = 0
    ...
    for row in range(len(board)):
        for col in range(len(board[row])):
            if board[row][col] == EMPTY:
                board[row][col] = X
                score = minimaxX(board, 0, False)        # check if input is depth or 0
                board[row][col] = EMPTY
                if(score > bestScore):
                    bestScore = score
                    bestMoveRow = row
...
def minimaxX(board, depth, isMaximizing):
    ...
    if isMaximizing:
        ...
        for row in range(len(board)):
            for col in range(len(board[row])):
                if (board[row][col] == EMPTY):
                    board[row][col] = O
                    score = minimaxX(board, depth + 1, True)
                    board[row][col] = EMPTY
                    if (score < bestScore):
                        bestScore = score
        return bestScore
